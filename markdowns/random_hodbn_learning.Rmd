...
evaluate_network <- function(og, size, n_vars, port){
  ...
  res <- dbnR::learn_dbn_struc(NULL, size, method = "psoho", f_dt = og$f_dt, n_inds = 300, n_it = 50,
                                       in_cte = 0.7, gb_cte = 0.5, lb_cte = 0.5,
                                       v_probs = c(10, 65, 25),  r_probs = c(-0.5, 1.5), score = "bic-g")
  b <- format(Sys.time() - b)
...
# psoho_t <- c(1.75, 9.62, 46.9, 2.25, 14.84, 72, 4.24, 18.65, 123.6, 5.55, 25.12, 166.8, 7.4, 42.26, 210)
# natPSOHO_t <- c(1.43, 8, 43.34, 1.6, 9.09, 50.7, 1.8, 10.34, 67.2, 1.84, 12.1, 68.4, 2.2, 13.56, 67.2)
networks <- c("10", "15", "20", "25", "30")
psoho_t <- c(20, 34.36, 58.16, 79.9, 132.1)
natPSOHO_t <- c(11, 15.2, 19.24, 22.81, 26.5)
