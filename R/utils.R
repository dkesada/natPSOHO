...
#' inside [1, max]. Because of this restriction, very low values of 'p' return
#' increasingly uniform populations in the interval [1, max].
#' 
#' @param p the parameter of the geometric distribution
#' @param max the maximum value allowed to be sampled
#' @return the sampled value
trunc_geom <- function(p, max){
  return(floor(log(1 - runif(1)*(1 - (1 - p)^max)) / log(1 - p)) + 1)
}
